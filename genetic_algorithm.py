...
class GeneticAlgorithm(LayoutDisplayMixin):
    ...
    def decode_layout(self, individual: List[Dict[str,Any]]) -> Tuple[List[Dict[str,Any]], int]:
        """
        Constrói um layout usando a técnica de Retângulos Livres.
        Cada peça é testada com rotações permitidas (0°/90°) para encaixe em um dos retângulos livres.
        Se não for possível encaixar, a peça é descartada e penalizada.
        Retorna: (layout_result, número de peças descartadas).
        """
    ...
    def evaluate(self, individual: List[Dict[str,Any]]) -> float:
        """
        Avalia um indivíduo decodificando seu layout via Retângulos Livres.
        O fitness é calculado como a área do bounding box do layout, somado a uma penalização para peças descartadas.
        """
    ...
    def run(self) -> List[Dict[str,Any]]:
        ...
        for opt in range(self.max_otimizacoes):
            for gen in range(self.numero_geracoes):
                ...
                if gen % 10 == 0:
                    print(f"Otimização {opt}, Geração {gen} - Melhor Fitness: {current_best}")
    ...
    def optimize_and_display(self):
        ...
        print("\nImpacto Econômico: O algoritmo otimiza o layout das peças, potencialmente reduzindo o desperdício de matéria-prima e energia.")
        print(f"Tempo de Processamento: {processing_time:.2f} segundos. Verifique se este tempo é adequado para uso prático.", flush=True)
